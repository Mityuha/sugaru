...
from sugaru import Plugin, sugarate
...
def test_sugarate_no_plugins_to_fetch(sugarate_args: SugarateArgs, mocker: Any) -> None:
    ...
    assert sugarate_args["object_loader"].mock_calls == []  # type: ignore
...
def test_sugarate_plugins_not_found(sugarate_args: SugarateArgs, mocker: Any) -> None:
    ...
    sugarate_args["object_loader"].configure_mock(side_effect=[None for _ in range(len(plugins))])  # type: ignore
    sugarate(**sugarate_args)

    assert sugarate_args["object_loader"].mock_calls == [mocker.call(pname, class_=Plugin, type_check=mocker.ANY) for pname in plugins]  # type: ignore
    assert sugarate_args["sugar_file_loader"].mock_calls == []  # type: ignore
    assert sugarate_args["final_file_writer"].mock_calls == []  # type: ignore
